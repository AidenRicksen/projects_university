...
[
  ...
  ;;The code above is taken from the epiDEM Basic model
  ;;(Yang & wilensky, 2011)

  p0                        ;; First agent to get infected; Patient Zero
  v0                        ;; Number of agent to be vaccinated
  gvallist                  ;; List of possible numbers for selecting vaccinated people
  i                         ;; iterator
  virlist                   ;; List of valid virus ID numbers

  hourly-peak-infected       ;; Highest number of infected people during the current hour
  overall-hourly-peak        ;; Overall highest hourly peak of infected people
  peak-hour                  ;; The hour when the overall peak occurred
  last-hour-tick             ;; The tick when the last hour started
  vaccinated-infected-count  ;; Number of vaccinated people infected at overall-hourly-peak
  non-vaccinated-infected-count ;; Number of non-vaccinated people infected at overall-hourly-peak
  max-infection-rate         ;; Maximum infection rate observed
]
...
people-own
[
  ;; These variables are taken from the epiDEM Basic model
  ;; (Yang & Wilensky, 2011)
  ...
  ;;adding the vaccination variable
  vaccinated?               ;; If true, the person is vaccinated
  nearby-uninfected         ;; List of turtles that are close and uninfected
  virus-id                  ;; ID of virus who infected the person
]

viruses-own
[
  infection-prob            ;; How likely the virus is to infect someone
  mutation-rate             ;; How likely the virus is to mutate
  mutation-impact           ;; How impactful a viral mutation is
  to-infect                 ;; People the virus has to infect
  person-id                 ;; Person who the virus infected
  j                         ;; iterator
]
...
to setup
  ...
  setup-people
  reset-ticks
  ;; The three lines above are taken from the epiDEM Basic model
  ;; (Yang & Wilensky, 2011)
  setup-virus
  vallist-set
  set hourly-peak-infected 0
  set overall-hourly-peak 0
  set peak-hour 0
  set last-hour-tick 0
  set vaccinated-infected-count 0
  set non-vaccinated-infected-count 0
  set max-infection-rate 0
end

to vallist-set
  set i 1
  let vallist [0]
  repeat initial-people - 1 [
    set vallist lput i vallist
    set i i + 1
  ]
  set gvallist vallist
end

to setup-virus
  create-viruses 1 [
    set infection-prob infection-chance
    set mutation-rate mutation-prob
    set mutation-impact random-float 2
    set person-id p0
    let tlist [0]
    set tlist lput initial-people tlist
    set virlist remove 0 tlist
    ht
  ]
end

to setup-people
  set p0 random initial-people  ;; Set Patient Zero
  create-people initial-people [
    ...
    set shape "person"
    set color white
    ;; the code from setup-people, except for the Patient Zero, is taken from the epiDEM Basic model
    ;; (Yang & wilensky)
    set virus-id 0
    set vaccinated? false

    ;;the calculation of the reovery time is taken from the epiDEM Basic model
    ;; (Yang & wilensky)
    ...
    ;; Ensure it lies between 0 and 2x average-recovery-time
    ...
    if (who = p0) [
      set infected? true ;; infected, susceptible and infection-lenght is taken from the epiDEM Basic model
                         ;; (Yang & wilensky)
      set susceptible? false
      set infection-length random recovery-time
      set virus-id initial-people
    ]
    assign-color
  ]

  repeat (initial-people * vac_rate) [
    set v0 random length gvallist
    ask people with [who = item v0 gvallist] [
      set vaccinated? true
      set susceptible? false
      set gvallist remove-item v0 gvallist
      assign-color
    ]
  ]
end
...
to assign-color  ;; turtle procedure
  ;; infected and cured code is taken from the epiDEM Basic model
  ;; (Yang & Wilensky, 2011)
  if infected? [ set color red ]
  if cured? [ set color green ]
  if vaccinated? and not infected? and not cured? [ set color blue ]
end
...
to go ;; This to go code was taken from the epiDEM Basic model
      ;; (Yang & Wilensky, 2011)
      ;; We changed the variable names, added the update of hourly metrics and infection rate
  if all? people [ not infected? ] [ stop ]

  ;; Update hourly metrics before infection and recovery updates
...
to move
  rt random-float 360
  fd 1
  ;;the code above from the to move section was taken from the epiDEM Basic model
  ;; (Yang & Wilensky, 2011)
  if virus-id != 0 and not cured? [
    ask virus virus-id [
      set xcor [xcor] of person person-id
      set ycor [ycor] of person person-id
    ]
  ]
end

to clear-count
  ;; to clear-count code is taken from the epiDEM Basic model
  ;; (Yang & Wilensky, 2011)
  set nb-infected 0
  set nb-recovered 0
end

to infect ;; to infect is partially taken from the epiDEM Basic model
          ;; (Yang & Wilensky, 2011)
  set nearby-uninfected nobody
  set nearby-uninfected (people-on neighbors) with [
    not infected? and not cured? and
    ;; adding the influence of vaccination and mutation on the infection
    (not vaccinated? or random 100 <= 100 - (vaccination-efficacy + (100 - 10 * [mutation-impact] of (one-of viruses-on neighbors))))
  ]
  ...
  if nearby-uninfected != nobody [
    ...
    ;; simulating the multiplication of viruses
    ask virus virus-id [
      if [nearby-uninfected] of person person-id != 0 [
        multiply
      ]
    ]
  ]
...
to maybe-recover ;; to maybe-recover is taken from the epiDEM Basic model
                 ;; (Yang & Wilensky, 2011)
...
to multiply
  set j 0
  set to-infect [who] of [nearby-uninfected] of person person-id
  repeat length to-infect [
    hatch 1 [
      set xcor random-xcor
      set ycor random-ycor
      set person-id item j to-infect
      set virlist lput who virlist
      if random 100 <= mutation-rate [
        set mutation-impact mutation-impact + random-float 2
      ]
      ask person person-id [
        set virus-id initial-people + length virlist - 1
      ]
    ]
    set j j + 1
  ]
...
to calculate-r0 ;; the calculation or r0 is taken from the epiDEM Basic model
                ;; (Yang & Wilensky, 2011)
  ...
  ifelse nb-infected-previous < 10 [ set beta-n 0 ] [
    set beta-n (new-infected / nb-infected-previous)
  ]

  ifelse nb-infected-previous < 10 [ set gamma 0 ] [
...
to update-hourly-metrics
  let current-infected count people with [infected?]

  ;; Update the current hourly peak if the current value is higher
  if current-infected > hourly-peak-infected [
    set hourly-peak-infected current-infected
  ]

  ;; Check if an hour (e.g., 60 ticks) has passed
  if ticks - last-hour-tick >= 60 [
    ;; Update the overall peak if this hour's peak is the highest
    if hourly-peak-infected > overall-hourly-peak [
      set overall-hourly-peak hourly-peak-infected
      set peak-hour ticks / 60  ;; Record the hour number

      ;; Recalculate counts for vaccinated and non-vaccinated individuals
      set vaccinated-infected-count count people with [infected? and vaccinated?]
      set non-vaccinated-infected-count count people with [infected? and not vaccinated?]
    ]

    ;; Reset the hourly peak and last-hour-tick for the next hour
    set hourly-peak-infected 0
    set last-hour-tick ticks
  ]
end

to update-infection-rate
  ;; Avoid division by zero on the first tick
  if ticks > 0 [
    let new-infections count people with [infected?] - nb-infected-previous

    ;; Calculate the infection rate
    let infection-rate new-infections / ticks
    if infection-rate > max-infection-rate [
      set max-infection-rate infection-rate
    ]

    ;; Update previous infected count
    set nb-infected-previous count people with [infected?]
  ]
end
...
218
597
Vaccination Status among Infected people
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -10649926 true "" "plot count people with [infected? and vaccinated?]"
"pen-1" 1.0 0 -5298144 true "" "plot count people with [infected? and not vaccinated?]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [mutation-impact] of max-one-of viruses [mutation-impact]"
...
636
499
Vaccinated and Infected at Peak
vaccinated-infected-count
17
1
11
...
## MODIFICATIONS OF THE CODE DONE BY THE GROUP:
- Added a system of vaccination: If a person is vaccinated, the chance of infection lowers to the vaccination-efficacy, which is a slider that can be controlled by the user. People are vaccinated initially, with a number set by the user (vac_rate)
- Added virus agents which connect via variables to the people agents. Thus, each person's virus can be tracked individually. A virus dies when the person infected with it recovers.
-Added mutation: the mutation-prob variable sets what the probability is of every single virus mutating with a replication. Each mutation can increase or decrease the initial infection probability using random numbers. The highest mutation impact can be monitored using the graph 'Mutation impact of best virus'.
-Removed the probability of infection in favor of a single 'Patient Zero': attempted to make the number of initially infected people variable, but this didn't work due to the virus agents having trouble synchronizing with the people agents.
@#$#@#$#@
default
true
0
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="true">
    ...
    <metric>overall-hourly-peak</metric>
    <metric>vaccinated-infected-count</metric>
    <metric>non-vaccinated-infected-count</metric>
    <metric>max-infection-rate</metric>
    ...
    <enumeratedValueSet variable="vac_rate">
      <value value="0.2"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="vaccination-efficacy">
      <value value="80"/>
    </enumeratedValueSet>
